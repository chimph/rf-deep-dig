...
import { readOwnedFriends, readOwnedFriendPages } from '../dist/owned-friends.js';
...
test('paged discovery reads only eight owned IDs per request and rechecks chain and cancellation between pages', async () => {
  const f = fixture(); f.state.balance = 19n;
  f.state.logs = Array.from({ length: 19 }, (_, i) => event(BigInt(i + 1), ZERO, OWNER, BigInt(i + 1)));
  f.state.generations = new Map(f.state.logs.map(log => [log.args.tokenId, 1]));
  f.state.generations.set(3n, 0);
  const abort = new AbortController();
  const pages = readOwnedFriendPages(f.client, OWNER, { signal: abort.signal });
  assert.equal(f.calls.length, 0, 'lazy until requested');
  let page = (await pages.next()).value;
  assert.equal(page.friends.length, 7); assert.equal(page.hiddenCount, 1); assert.equal(page.remainingCount, 11);
  assert.equal(f.calls.filter(call => call.functionName === 'ownerOf').length, 8);
  page = (await pages.next()).value;
  assert.equal(page.friends.length, 15); assert.equal(page.remainingCount, 3);
  assert.equal(f.calls.filter(call => call.method === 'getLogs').length, 2, 'history snapshot reused');
  abort.abort();
  await assert.rejects(pages.next(), { name: 'AbortError' });
  assert.equal(f.calls.filter(call => call.functionName === 'ownerOf').length, 16);
  const changed = readOwnedFriendPages(f.client, OWNER);
  await changed.next(); f.state.chain = 1;
  await assert.rejects(changed.next(), /chain 4663/);
});
