...
test('sound preference accepts only a boolean in the active private session', async () => {
  const { port1, port2 } = new MessageChannel();
  const preview = createGamePreview(definition, { stake: 10n * RF, rfBalance: 20n * RF, friendId: 5n });
  const updates = [];
  const host = bindGameFrame(port1, { client: preview.client, authorize: async () => {}, onSoundMuted: muted => updates.push(muted) });
  const frame = createFrameGameClient(port2, definition);
  try {
    host.setPaused(true); // A harmless UI preference can change in a menu.
    frame.setSoundMuted(false); frame.setSoundMuted(true);
    port2.postMessage({ type: 'friendsdk:sound-muted', muted: 'false', key: 'deep-dig.offline.v4' });
    port2.postMessage({ type: 'friendsdk:sound-muted', muted: { balance: 999 } });
    await frame.client.read(); assert.deepEqual(updates, [false, true]);
    frame.close(); frame.setSoundMuted(false); assert.deepEqual(updates, [false, true]);
    assert.equal((await preview.client.read()).rfBalance, 20n * RF);
  } finally { host.close(); frame.close(); }
});
test('picker shortcut opens only the trusted menu and ignores paused or closed sessions', async () => {
  const { port1, port2 } = new MessageChannel();
  const preview = createGamePreview(definition, { stake: 10n * RF, rfBalance: 20n * RF, friendId: 5n });
  let opened = 0;
  const host = bindGameFrame(port1, { client: preview.client, authorize: async () => { throw Error('No actions expected'); }, onOpenFriends: () => { opened++; } });
  const frame = createFrameGameClient(port2, definition);
  try {
    frame.openFriendSelector(); await frame.client.read(); assert.equal(opened, 1);
    host.setPaused(true); frame.openFriendSelector(); await frame.client.read(); assert.equal(opened, 1);
    host.setPaused(false); frame.openFriendSelector(); await frame.client.read(); assert.equal(opened, 2);
    assert.equal((await preview.client.read()).rfBalance, 20n * RF);
    frame.close(); frame.openFriendSelector(); assert.equal(opened, 2);
  } finally { host.close(); frame.close(); }
});
