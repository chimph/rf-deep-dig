...
import { readFile, writeFile, mkdir, realpath, stat, lstat, readdir, rename } from 'node:fs/promises';
...
createRoot(document.getElementById('root')).render(<GameHost definition={definition} frameUrl="./game.html" deployment={deployment} friendPickerArtwork={gameJson.runtime?.friendPickerArtwork === true}/>);`,
  } });
  let child;
  try {
