...
export async function installFixture(page, origin, { artworkCall, initialChain = "0x1237", ownedIds = [7730n] } = {}) {
  const ownerTokens = owner => owner.toLowerCase() === OWNER.toLowerCase() ? ownedIds : [3412n];
  ...
  async function answer(request) {
    ...
    if (request.method === "eth_chainId") result = "0x1237";
      ...
      result = filter.topics[1] ? [] : ownerTokens(owner).map((id, index) => ({ address: COLLECTION, blockNumber: "0x10", blockHash: padHex("0x10", { size: 32 }),
        data: "0x", logIndex: `0x${index.toString(16)}`, transactionHash: padHex("0x1234", { size: 32 }), transactionIndex: "0x0", removed: false,
      ...
      if (request.params[0].to.toLowerCase() !== COLLECTION.toLowerCase()) {
        ...
        let value;
        if (functionName === "balanceOf") value = state.mode === "unowned" ? 0n : BigInt(ownerTokens(args[0]).length);
        else {
          assert([...ownedIds, 3412n].includes(args[0]), "Do not enumerate token IDs or scan the collection");
          if (functionName === "ownerOf") {
            state.ownerReads++;
            value = state.mode === "owner-changed" && state.ownerReads > 1 ? SECOND_OWNER : ownedIds.includes(args[0]) ? OWNER : SECOND_OWNER;
...
export async function createArtworkFixture() {
  const source = await readFile(new URL("../games/deep-dig/canonical-sprites.ts", import.meta.url), "utf8");
