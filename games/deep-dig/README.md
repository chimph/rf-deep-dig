...
9. **Reveal all ordinary tiles and recover an orb on the current level to unlock free descent.** An orb recovered earlier on that level counts. Collecting every visible ground find is optional; uncollected rewards return to the pool when you leave. A new level requires its own orb. Before descent, a shared “Progress to level N?” prompt shows the destination level’s stake-scaled reward table and explains free entry and risk to the entire displayed haul. Cancel keeps the current board; Continue moves to the next level. Depth 3 is the bottom. Uncovering its last ordinary tile shows a “Level 3 cleared!” congratulations screen with one Continue button. Continue closes the message without ending the run or banking the haul; remaining finds and orbs are still available. Additional orb attempts on the same board still pay when successful.
