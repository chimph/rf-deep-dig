...
export type GameFrameProps = {
  ...
  friendsRemainingCount?: number; onLoadMoreFriends?: () => void;
  renderFriendArtwork?: (friend: GameFriend) => ReactNode;
  /** Increment to open the trusted picker from an in-game menu. */
  friendPickerRequest?: number;
...
export function GameFrame({ children, friends, selectedFriendId, onSelectFriend, friendsLoading, friendsError, friendsEmptyMessage = "No playable Friends found.", friendsHiddenCount = 0, friendsRemainingCount = 0, onLoadMoreFriends, renderFriendArtwork, friendPickerRequest = 0, onConnect, wallet, confirmation, connection, walletActions, selectionMode = "picker", mode, onMenuChange }: GameFrameProps) {
  const [menu, setMenu] = useState<"friends" | "wallet" | null>(null);
  const previousPickerRequest = useRef(friendPickerRequest);
  useEffect(() => {
    if (friendPickerRequest !== previousPickerRequest.current) setMenu("friends");
    previousPickerRequest.current = friendPickerRequest;
  }, [friendPickerRequest]);
  ...
  return <section className="rf-game-frame" aria-label="Game container" data-mode={mode}>
    ...
    {confirmation ? <GameMenu title={confirmation.title} onClose={confirmation.busy ? undefined : confirmation.onCancel}
      ...
      <div className="rf-frame-friends">{friends.map(value => <button type="button" key={value.id.toString()} aria-label={value.label} aria-pressed={value.id === selectedFriendId} onClick={() => { onSelectFriend?.(value.id); setMenu(null); }}>{renderFriendArtwork?.(value)}<strong>{value.label}</strong><small>{value.kind === "sample" ? "Sample · no ownership claim" : "Hardwired Generations"}</small></button>)}</div>
      {friendsRemainingCount > 0 && onLoadMoreFriends && <button type="button" disabled={friendsLoading} onClick={onLoadMoreFriends}>{friendsLoading ? "Loading more Friends…" : "Load more Friends"}</button>}
      ...
      {!friendsLoading && !friendsError && !friends.length && !friendsRemainingCount && friendsEmptyMessage && <p>{friendsEmptyMessage}</p>}
