...
export function bindGameFrame(port: MessagePort, options: {
  ...
  port.onmessage = async ({ data }: MessageEvent<unknown>) => {
    ...
    const request = data as { type?: unknown; id?: unknown; method?: unknown; args?: unknown; muted?: unknown };
    if (request.type === 'friendsdk:sound-muted') {
      if (typeof request.muted === 'boolean') options.onSoundMuted?.(request.muted);
      return;
    }
    if (request.type === 'friendsdk:open-friends') {
      if (!busy && !paused) options.onOpenFriends?.();
      return;
    }
...
export function createFrameGameClient(port: MessagePort, definition: ChanceGameDefinition, onPause?: (paused: boolean) => void, mode: GameClient["mode"] = "preview") {
  ...
  return { client, close, setSoundMuted(muted: boolean) { if (alive && typeof muted === 'boolean') port.postMessage({ type: 'friendsdk:sound-muted', muted }); }, openFriendSelector() { if (alive) port.postMessage({ type: 'friendsdk:open-friends' }); } };
}
