 ...
 */
export async function* readOwnedFriendPages(
  client: OwnedFriendsClient, account: Address, options: OwnedFriendsOptions = {},
): AsyncGenerator<Readonly<{ friends: readonly OwnedFriend[]; blockNumber: bigint; hiddenCount: number; remainingCount: number }>> {
  ...
  if (balance === 0n) {
    await checkChain();
    yield Object.freeze({ friends: Object.freeze([]), blockNumber, hiddenCount: 0, remainingCount: 0 });
    return;
  }
  ...
  for (let offset = 0; offset < ids.length; offset += 8) {
    ...
    await checkChain();
    const processed = Math.min(offset + 8, ids.length);
    yield Object.freeze({ friends: Object.freeze([...friends]), blockNumber,
      hiddenCount: processed - friends.length, remainingCount: ids.length - processed });
  }
}

/** Compatibility API for consumers that need the complete discovery snapshot. */
export async function readOwnedFriends(client: OwnedFriendsClient, account: Address, options: OwnedFriendsOptions = {}): Promise<Readonly<{ friends: readonly OwnedFriend[]; blockNumber: bigint; hiddenCount: number }>> {
  let result: { friends: readonly OwnedFriend[]; blockNumber: bigint; hiddenCount: number } | undefined;
  for await (const page of readOwnedFriendPages(client, account, options)) {
    result = { friends: page.friends, blockNumber: page.blockNumber, hiddenCount: page.hiddenCount };
  }
