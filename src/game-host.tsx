...
import { readGameSoundMuted, writeGameSoundMuted } from "./game-sound-preference.js";
import { bindGameFrame, type GameArguments, type GameMethod } from "./frame-bridge.js";
import { FriendPickerArtwork } from "./friend-picker-artwork.js";
import { createGenerationSpriteReader, type GenerationSprites } from "./generation-sprites.js";
...
import { readOwnedFriendPages, type OwnedFriendsClient, type OwnedFriend } from "./owned-friends.js";
...
export type GameHostProps = {
  ...
  frameUrl: string;
  /** Show canonical previews in the trusted owned-Friend picker. */
  friendPickerArtwork?: boolean;
...
function WalletViewport({ session, publicClient, ...props }: Omit<GameHostProps, "walletProvider" | "publicClient"> & {
  ...
  const [discovery, setDiscovery] = useState<{
    client: OwnedFriendsClient; session: FriendWalletSession; revision: number; attempt: number;
    friends: readonly OwnedFriend[]; hiddenCount: number; remainingCount: number; loading: boolean; error?: string;
  } | null>(null);
  const loadMore = useRef<(() => void) | null>(null);
  const artworkReader = useMemo(() => {
    const source = createGenerationSpriteReader(publicClient);
    // Retain each loaded gallery card's immutable artwork for this wallet revision.
    // Reuse in-flight reads too, including a selection made before its card finishes.
    const reads = new Map<bigint, Promise<GenerationSprites>>();
    return {
      read(id: bigint) {
        let pending = reads.get(id);
        if (!pending) {
          pending = source.read(id); reads.set(id, pending);
          const current = pending;
          void pending.catch(() => { if (reads.get(id) === current) reads.delete(id); });
        }
        return pending;
      },
      clear() { reads.clear(); source.clear(); },
    };
  }, [publicClient, wallet.revision, attempt]);
  ...
  useEffect(() => {
    ...
    const pages = readOwnedFriendPages(publicClient, wallet.account, { signal: controller.signal });
    const identity = { client: publicClient, session, revision: wallet.revision, attempt };
    let loading = false, finished = false;
    setDiscovery({ ...identity, friends: [], hiddenCount: 0, remainingCount: 0, loading: true });
    const next = async () => {
      if (loading || finished || controller.signal.aborted) return;
      loading = true;
      setDiscovery(previous => previous ? { ...previous, loading: true } : previous);
      try {
        const page = await pages.next();
        if (controller.signal.aborted) return;
        finished = !!page.done || page.value.remainingCount === 0;
        if (!page.done) setDiscovery({ ...identity, ...page.value, loading: false });
      } catch (error) {
        finished = true;
        if (!controller.signal.aborted) setDiscovery({ ...identity, friends: [], hiddenCount: 0, remainingCount: 0, loading: false,
          error: error instanceof Error ? error.message : "Could not load your Friends. Try again." });
      } finally { loading = false; }
    };
    loadMore.current = () => { void next(); };
    void next();
    return () => { controller.abort(); loadMore.current = null; void pages.return(undefined); };
  ...
  return <ConnectedViewport {...props} readFriendArtwork={props.friendPickerArtwork ? artworkReader.read : undefined} selectedFriend={friend} account={wallet.account} chainId={wallet.chainId}
    publicClient={publicClient} revision={wallet.revision} walletClient={walletClient} assertActive={assertWalletActive} picker={{ friends, onSelectFriend: setSelected, connection,
      friendsLoading: wallet.status === "connected" && (!valid || valid.loading), friendsError: valid?.error,
      friendsRemainingCount: valid?.remainingCount, onLoadMoreFriends: () => loadMore.current?.(),
      renderFriendArtwork: props.friendPickerArtwork ? value => <FriendPickerArtwork key={`${wallet.revision}:${value.id}`} friendId={value.id} reader={artworkReader} /> : undefined,
...
type Picker = Pick<GameFrameProps, "friends" | "onSelectFriend" | "connection" | "friendsLoading" | "friendsError" | "friendsEmptyMessage" | "friendsHiddenCount" | "onConnect" | "friendsRemainingCount" | "onLoadMoreFriends" | "renderFriendArtwork">;
...
type ArtworkReader = (id: bigint) => Promise<GenerationSprites>;

function ConnectedViewport({ readFriendArtwork, definition, selectedFriend, account, chainId, publicClient, frameUrl, revision = 0, picker, deployment, walletClient, assertActive }: ConnectedGameHostProps & { picker?: Picker; readFriendArtwork?: ArtworkReader }) {
  ...
  return <EligibilityGate readFriendArtwork={readFriendArtwork} key={sessionKey} definition={definition} picker={picker} friend={selectedFriend} account={account} chainId={chainId} publicClient={publicClient}
    frameUrl={frameUrl} ledgers={ledgers} deployment={deployment} walletClient={walletClient} assertActive={assertActive} />;
}

function EligibilityGate({ readFriendArtwork, definition, picker, friend, account, chainId, publicClient, frameUrl, ledgers, deployment, walletClient, assertActive }: {
  definition: ChanceGameDefinition; picker?: Picker; readFriendArtwork?: ArtworkReader;
  ...
  const [verification, setVerification] = useState<{ client: GenerationIdentityClient; eligible: boolean; walletAddress?: string; artwork?: GenerationSprites; error?: string } | null>(null);
  useEffect(() => {
    let alive = true;
    ...
    void Promise.all([
      readGenerationEligibility(publicClient, friend.id, account as Address),
      readFriendArtwork?.(friend.id).catch(() => undefined),
    ]).then(async ([result, artwork]) => {
      ...
      if (alive) setVerification({ client: publicClient, eligible: result.eligible === true, walletAddress, artwork,
    ...
    return () => { alive = false; };
  }, [publicClient, friend.id, account, chainId, attempt, readFriendArtwork]);
  ...
  if (!checked?.eligible) return <GameFrame friendPickerRequest={friendPickerRequest} mode={deployment ? "live" : "preview"} selectionMode={picker ? "picker" : "host"} friends={[friend]} selectedFriendId={friend.id} {...picker}>
    <div className="rf-runtime-status" role={checked?.error ? "alert" : "status"}>
      ...
      {picker && <button type="button" onClick={() => setFriendPickerRequest(value => value + 1)}>Choose Friend</button>}
  ...
  if (deployment) {
    ...
    return <EmbeddedSession friendArtwork={checked.artwork} key={frameUrl} picker={picker} friend={{ ...friend, kind: "owned", walletAddress: checked.walletAddress }}
  ...
  return <EmbeddedSession friendArtwork={checked.artwork} key={frameUrl} picker={picker} friend={{ ...friend, kind: "owned", walletAddress: checked.walletAddress }} client={client} definition={definition} frameUrl={frameUrl} />;
}

function EmbeddedSession({ friendArtwork, friend, client, definition, live, frameUrl, picker }: {
  friendArtwork?: GenerationSprites; friend: GameFriend; client?: GameClient; definition: ChanceGameDefinition; live?: LiveGameOptions; frameUrl: string; picker?: Picker;
}) {
  ...
  useLayoutEffect(() => {
    ...
    function ready(event: MessageEvent) {
      ...
      const connection = bindGameFrame(channel.port1, { client: activeClient, authorize,
        onSoundMuted(muted) { if (alive) writeGameSoundMuted(definition.name, muted); },
        onOpenFriends() { if (alive && picker && !fundingRef.current) setFriendPickerRequest(value => value + 1); },
      ...
      frame.contentWindow!.postMessage({ type: "friendsdk:init", documentId, handshakeId, friendId: friend.id, mode: activeClient.mode, soundMuted: readGameSoundMuted(definition.name),
        friendArtwork: friendArtwork ? { tokenId: friendArtwork.tokenId, familyId: friendArtwork.familyId, seed: friendArtwork.seed, frames: friendArtwork.frames } : undefined }, "*", [channel.port2]);
  ...
  }, [client, definition, friend.id, attempt, friendArtwork]);
  ...
  return <GameFrame mode={mode} selectionMode={picker ? "picker" : "host"} friends={[friend]} selectedFriendId={friend.id}
    ...
    confirmation={confirmation} friendPickerRequest={friendPickerRequest} onMenuChange={onMenuChange} {...picker}>
